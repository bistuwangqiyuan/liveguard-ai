"""
render_docx.py
==============

把 `投智云InvestMind_商业计划书_v1.0.md` 渲染为同名 .docx：

  1. 用 python-docx 生成中文 reference.docx（标题样式 / 字体 / 页眉页脚）
  2. 用 pandoc --reference-doc=reference.docx 转换 markdown → docx
"""

from __future__ import annotations
import subprocess
from pathlib import Path
from docx import Document
from docx.shared import Pt, Cm, RGBColor
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx.oxml.ns import qn
from docx.oxml import OxmlElement

ROOT = Path(__file__).parent.parent.parent
BUILD = ROOT / "investmind" / "bp_build"
REF = BUILD / "reference.docx"
MD = ROOT / "投智云InvestMind_商业计划书_v1.0.md"
OUT = ROOT / "投智云InvestMind_商业计划书_v1.0.docx"

CHINESE_FONT_LATIN = "Inter"
CHINESE_FONT_CJK   = "Noto Sans CJK SC"
HEADING_FONT       = "Noto Sans CJK SC"

PRIMARY = "1F6FFF"
INK     = "0B0F1A"


def set_cjk_font(target, name: str = CHINESE_FONT_CJK):
    """target: 可以是 run 或 rPr 节点。"""
    if hasattr(target, "_element"):
        rPr = target._element.get_or_add_rPr()
    else:
        rPr = target
        if rPr is None:
            return
    rFonts = rPr.find(qn("w:rFonts"))
    if rFonts is None:
        rFonts = OxmlElement("w:rFonts")
        rPr.insert(0, rFonts)
    for attr in ("w:eastAsia", "w:hAnsi", "w:cs", "w:ascii"):
        rFonts.set(qn(attr), name)


def style_run(run, font_name: str = CHINESE_FONT_LATIN, size_pt: float = 11,
              bold: bool = False, color_hex: str = INK):
    run.font.name = font_name
    run.font.size = Pt(size_pt)
    run.bold = bold
    run.font.color.rgb = RGBColor.from_string(color_hex)
    set_cjk_font(run, CHINESE_FONT_CJK)


def build_reference():
    doc = Document()

    section = doc.sections[0]
    section.top_margin = Cm(2.4)
    section.bottom_margin = Cm(2.4)
    section.left_margin = Cm(2.6)
    section.right_margin = Cm(2.6)

    styles = doc.styles
    normal = styles["Normal"]
    normal.font.name = CHINESE_FONT_LATIN
    normal.font.size = Pt(10.5)
    rPr_normal = normal.element.find(qn("w:rPr"))
    if rPr_normal is None:
        rPr_normal = OxmlElement("w:rPr")
        normal.element.append(rPr_normal)
    set_cjk_font(rPr_normal, CHINESE_FONT_CJK)

    for h, size, color in [
        ("Heading 1", 22, PRIMARY),
        ("Heading 2", 16, INK),
        ("Heading 3", 13, INK),
        ("Heading 4", 11, INK),
    ]:
        st = styles[h]
        st.font.name = HEADING_FONT
        st.font.size = Pt(size)
        st.font.bold = True
        st.font.color.rgb = RGBColor.from_string(color)
        rPr_h = st.element.find(qn("w:rPr"))
        if rPr_h is None:
            rPr_h = OxmlElement("w:rPr")
            st.element.append(rPr_h)
        set_cjk_font(rPr_h, HEADING_FONT)

    header = section.header
    p = header.paragraphs[0]
    run = p.add_run("投智云 InvestMind AI · 商业计划书 v1.0")
    style_run(run, size_pt=9, color_hex="8A93A6")
    p.alignment = WD_ALIGN_PARAGRAPH.RIGHT

    footer = section.footer
    p = footer.paragraphs[0]
    run = p.add_run("CONFIDENTIAL · 仅授权对象阅读 · © 2026 InvestMind AI")
    style_run(run, size_pt=8.5, color_hex="8A93A6")
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER

    cover = doc.add_paragraph()
    cover.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r = cover.add_run("\n\n\n\n投智云 InvestMind AI")
    style_run(r, font_name=HEADING_FONT, size_pt=36, bold=True, color_hex=PRIMARY)
    cover.add_run("\n")
    r = cover.add_run("早期股权投资可行性研究与决策辅助平台\n商业计划书 v1.0")
    style_run(r, font_name=HEADING_FONT, size_pt=18, color_hex=INK)
    cover.add_run("\n")
    r = cover.add_run("\n2026 年 04 月 30 日")
    style_run(r, size_pt=12, color_hex="8A93A6")

    doc.save(REF)
    print(f"✓ reference.docx 写入 {REF}")


def render_docx():
    print(f"── pandoc render: {MD.name} → {OUT.name} ──")
    cmd = [
        "pandoc",
        str(MD),
        "-o", str(OUT),
        "--reference-doc", str(REF),
        "--toc", "--toc-depth=2",
        "--from", "markdown+pipe_tables+task_lists",
        "--to", "docx",
        "--standalone",
        "--resource-path", str(ROOT),
    ]
    print(" ".join(cmd))
    res = subprocess.run(cmd, capture_output=True, text=True)
    if res.returncode != 0:
        print("STDERR:", res.stderr)
        raise SystemExit(res.returncode)
    print("STDOUT:", res.stdout[:2000] if res.stdout else "(empty)")
    print(f"✓ 写入 {OUT}  大小 {OUT.stat().st_size/1024:.1f} KB")


if __name__ == "__main__":
    build_reference()
    render_docx()
